        ...
        if (type === 'tokens') {
            ...
            return `
                <div class="interactive-card tool-widget">
                    <div class="card-badge" style="background:#d97757">${esc(t('widget.token.badge'))}</div>
                    <h3>🧮 ${esc(t('widget.token.title'))}</h3>
                    <p style="color: rgba(255,255,255,0.7)">${esc(t('widget.token.desc'))}</p>
                    <textarea class="tool-input" id="token-input" rows="3" placeholder="${esc(t('widget.token.placeholder'))}" oninput="Engine.calculateTokens(this.value)"></textarea>
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <span style="font-size: 2rem; font-weight: 800;" id="token-count">0</span>
                            <span style="opacity: 0.7">${esc(t('widget.token.tokens'))}</span>
                        </div>
                        <div style="text-align: right;">
                            <span style="font-size: 1.2rem; font-weight: 700;" id="word-count">0</span>
                            <span style="opacity: 0.7">${esc(t('widget.token.words'))}</span>
                        </div>
                    </div>
                </div>
            `;
